...
def edit_expense(expense_id: int, date: str = None, amount: float = None, category: str = None, subcategory: str = None, note: str = None):
    ...
    with sqlite3.connect(DB_PATH) as c:
        cur = c.execute("SELECT id, date, amount, category, subcategory, note FROM expenses WHERE id = ?", (expense_id,))
        row = cur.fetchone()
        if not row:
            return {"status": "error", "message": f"No expense found with id {expense_id}"}
        ...
        c.execute(
            ...
            (updated["date"], updated["amount"], updated["category"], updated["subcategory"], updated["note"], expense_id)
        )
